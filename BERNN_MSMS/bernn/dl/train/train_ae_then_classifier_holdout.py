        ...
        while h < self.args.n_repeats:
            ...
            if h == 1:
                for epoch in range(0, self.args.warmup):
                    ...
                    if epoch == self.args.warmup - 1 and warmup:
                        # When the warnup counter gets to
                        if self.args.early_warmup_stop != 0:
                            try:
                                ae.load_state_dict(torch.load(f'{self.complete_log_path}/model_{h}.pth'))
            ...
            for epoch in range(0, self.args.n_epochs):
                ...
                with torch.no_grad():
                    ...
                    closs, _, _ = self.loop('train', optimizer_ae, ae, sceloss, loaders['train'], lists, traces, nu=nu)
            ...
            with torch.no_grad():
                for group in list(data['inputs'].keys()):
                    # if group in ['all', 'all_pool']:
                    #     continue
                    closs, best_lists, traces = self.loop(group, optimizer_c, ae, sceloss, loaders[group], best_lists, traces, nu=0, mapping=False)  # -1
            if self.log_neptune:
