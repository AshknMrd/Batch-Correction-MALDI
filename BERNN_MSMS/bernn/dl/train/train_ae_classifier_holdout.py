            ...
            if len(event_acc.Tags()['tensors']) > 2 and self.load_tb:
                    ...
                    with torch.no_grad():
                        for group in list(data['inputs'].keys()):
                            if group in ['all', 'all_pool']:
                                continue
                            closs, lists, traces = self.loop(group, optimizer_ae, ae, sceloss, loaders[group], lists, traces, nu=0)
                        # closs, _, _ = self.loop('train', optimizer_ae, ae, sceloss, loaders['train'], lists, traces, nu=0)
                    # Below is the loop for all sets
                    # with torch.no_grad():
                    #     for group in list(data['inputs'].keys()):
                    #         if group in ['all', 'all_pool']:
                ...
                with torch.no_grad():
                    for group in list(data['inputs'].keys()):
                        # if group in ['all', 'all_pool']:
                        #     continue
                        closs, best_lists, traces = self.loop(group, None, ae, sceloss, loaders[group], best_lists, traces, nu=0, mapping=False)
                # if self.log_neptune:
