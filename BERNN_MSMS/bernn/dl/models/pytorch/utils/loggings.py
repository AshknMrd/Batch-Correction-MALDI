...
class TensorboardLoggingAE:
    ...
    def logging(self, traces, metrics):
        with self.writer.as_default():
            ...
            step = len(traces['rec_loss'])
            tf.summary.scalar('loss/rec', float(np.squeeze(traces['rec_loss'][-1])), step=step)
            tf.summary.scalar('loss/dom', float(np.squeeze(traces['dom_loss'][-1])), step=step)
            try:
                tf.summary.scalar('acc/dom', float(np.squeeze(traces['dom_acc'][-1])), step=step)
            except:
                tf.summary.scalar('acc/dom', traces['dom_acc'][-1], step=step)

            # for g in ['train', 'valid', 'test']:
            #     tf.summary.scalar(f'{g}/loss', float(np.squeeze(traces[g]['closs'][-1])) if traces[g]['closs'] else float('nan'), step=step)              
            #     tf.summary.scalar(f'{g}/acc', float(np.squeeze(traces[g]['acc'][-1])) if traces[g]['acc'] else float('nan'), step=step)
            #     tf.summary.scalar(f'{g}/top3', float(np.squeeze(traces[g]['top3'][-1])) if traces[g]['top3'] else float('nan'), step=step)
            #     tf.summary.scalar(f'{g}/mcc', float(np.squeeze(traces[g]['mcc'][-1])) if traces[g]['mcc'] else float('nan'), step=step)
            ...
            # tf.summary.scalar('enc b_euclidean/tot_eucl', float(np.squeeze(traces['enc b_euclidean/tot_eucl'])), step=1),
            # tf.summary.scalar('enc qc_dist/tot_eucl', float(np.squeeze(traces['enc qc_dist/tot_eucl'])), step=1),
            # tf.summary.scalar('enc qc_aPCC', float(np.squeeze(traces['enc qc_aPCC'])), step=1),
            # tf.summary.scalar('rec b_euclidean/tot_eucl', float(np.squeeze(traces['rec b_euclidean/tot_eucl'])), step=1),
            # tf.summary.scalar('rec qc_dist/tot_eucl', float(np.squeeze(traces['rec qc_dist/tot_eucl'])), step=1),
            # tf.summary.scalar('rec qc_aPCC', float(np.squeeze(traces['rec qc_aPCC'])), step=1),
