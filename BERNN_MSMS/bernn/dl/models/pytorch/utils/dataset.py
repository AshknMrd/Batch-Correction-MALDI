...
class MSDataset3(Dataset):
    def __init__(self, data, meta, names=None, labels=None, batches=None, 
        ...
        self.sets = list(sets) if sets is not None else None
    ...
    def __getitem__(self, idx):
        ...
        try:
            set_name = self.sets[idx] if self.sets is not None and len(self.sets) > idx else 'unknown'
        except:
            set_name = 'unknown'
        try:
...
class MSDataset4(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.labels is not None:
            ...
            batch = self.batches[idx]
            set1 = self.sets[idx] if len(self.sets) > 0 else 'unknown'
            try:
                name = self.names.iloc[idx] if isinstance(self.names, pd.Series) else str(self.names[idx])
            except Exception as e:
                print(f"Error in names: {e}")
                name = str(self.names[idx])
            meta_to_rec = self.meta.iloc[idx].to_numpy() if isinstance(self.meta, pd.DataFrame) else self.meta[idx]
        else:
            label = label_name = batch = name = None
        ...
        if self.random_recs:
            ...
            to_rec = self.cultpures[label_name].copy() if label_name != 'blanc' else self.samples[idx].copy()
        ...
        if (self.triplet_dloss in ['revTriplet', 'inverseTriplet']) and len(self.unique_batches) > 1:
            ...
            pos_batch_sample = neg_batch_sample = meta_pos_batch_sample = meta_neg_batch_sample = self.samples[idx].copy()
        ...
        return (
            x, meta_to_rec, name, label, batch, to_rec, not_to_rec,
            pos_to_rec, neg_to_rec, pos_batch_sample, neg_batch_sample,
