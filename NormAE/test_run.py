...
def get_data(log_transform: bool = False) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    info_df = info_df.loc[:, ["injection.order", "batch", "class"]] # extract the useful information from y_file

    # batch labels need to beginning from zero (-1 is not acceptable)
    # info_df.replace({"class": {"Subject": 1, "QC": 0}}).infer_objects(copy=False, inplace=True) # digitize class
